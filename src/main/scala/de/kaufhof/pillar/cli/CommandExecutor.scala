...
object CommandExecutor {
  implicit private val migratorConstructor: (Registry, StatementRegistry, Reporter, String, Boolean) => Migrator = Migrator.apply

  def apply(): CommandExecutor = new CommandExecutor()
}

class CommandExecutor(implicit val migratorConstructor: (Registry, StatementRegistry, Reporter, String, Boolean) => Migrator) {
  def execute(command: Command, reporter: Reporter) {
    val migrator = migratorConstructor(command.registry, command.preparedStatements, reporter, command.appliedMigrationsTableName, command.debug)
